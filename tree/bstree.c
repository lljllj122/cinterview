...
static inline BSTreeNode *BSTreeNode_successor(BSTreeNode *node)
{
    if (node->right) {
        return BSTree_find_min(node->right);
    }

    BSTreeNode *p = node->parent;
...
static inline BSTreeNode *BSTreeNode_predecessor(BSTreeNode *node)
{
    if (node->left) {
        return BSTree_find_max(node->left);
    }
...
static inline void BSTree_replace_node_in_parent(BSTree *map, BSTreeNode *node, BSTreeNode *new)
{
    if (node->parent) {
